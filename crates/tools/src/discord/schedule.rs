...
use serenity::{
    all::{CreateScheduledEvent, EditScheduledEvent, ScheduledEventId},
    http::Http,
};
...
pub struct GetDiscordScheduledEvents {
    http: Arc<Http>,
}
pub struct CreateDiscordScheduledEvent {
    http: Arc<Http>,
}
pub struct ModifyDiscordScheduledEvent {
    http: Arc<Http>,
}
pub struct DeleteDiscordScheduledEvent {
    http: Arc<Http>,
}

impl GetDiscordScheduledEvents {
    pub fn new(http: Arc<Http>) -> Self {
        Self { http }
    }
}
impl CreateDiscordScheduledEvent {
    pub fn new(http: Arc<Http>) -> Self {
        Self { http }
    }
}
impl ModifyDiscordScheduledEvent {
    pub fn new(http: Arc<Http>) -> Self {
        Self { http }
    }
}
impl DeleteDiscordScheduledEvent {
    pub fn new(http: Arc<Http>) -> Self {
        Self { http }
    }
}
...
impl Tool for CreateDiscordScheduledEvent {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let Some(start_time) = args.get("start_time").and_then(parse_timestamp) else {
            return Ok(err("start_time is required (RFC3339)"));
        };
        let Some(kind) = args.get("kind").and_then(parse_scheduled_event_type) else {
            return Ok(err("kind is required"));
        };
